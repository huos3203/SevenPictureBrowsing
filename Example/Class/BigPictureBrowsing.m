...
-(instancetype)initWithImageURLArray:(NSArray *)array andSelectedIndex:(NSInteger)index{
    ...
    if (self) {
        ...
        for (NSUInteger i = 0; i < self.imageURLArray.count; i++) {
            ...
            UIImageView * imageView = [[UIImageView alloc] initWithFrame:CGRectMake(i * ScreenWidth, (SCREENHEIGHT - ScreenWidth) / 2, ScreenWidth, ScreenWidth)];
           ...
           [imageView setImageWithURL:url placeholderImage:[UIImage imageNamed:@"JHLivePlayBundle.bundle/pic_ful.tiff"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType) {
